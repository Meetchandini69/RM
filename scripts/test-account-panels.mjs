...
try {
 ...
 let r=await call('registrations',member);assert.equal(r.status,200,JSON.stringify(r.data));const id=r.data.id;assert.equal(r.data.listing,'free');assert.equal(r.data.photos.length,0);assert.equal(r.data.notificationStatus,'Sent');assert.equal(telegramMessages,beforeNotifications+1);
 ...
 assert.equal((await call('registration/admin/payment-settings',{telegram:'@team_payments'})).status,401);
 assert.equal((await call('registration/admin/payment-settings',{telegram:'https://evil.test'},admin)).status,400);
 assert.equal((await call('registration/admin/payment-settings',{telegram:'@team_payments'},admin)).status,200);
 assert.equal((await call('registration/membership',null,mc)).data.telegram,'team_payments');
 ...
 assert.ok((await call('registration/membership',null,mc)).data.membership);
 assert.equal((await call('registration/me',null,mc)).data.listing,'quarterly');
 ...
 await client.query("UPDATE profile_boosts SET expires=now()-interval '1 day' WHERE id=$1",[bid]);
 assert.equal((await call(`profiles/member-${id}`)).status,404);
 assert.equal((await call('registration/profile',complete,mc,'PUT')).status,403);
