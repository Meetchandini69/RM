...
export function SeoAdmin(){
 ...
 <div className="rounded-2xl border border-foreground/15 bg-card p-5">
  ...
  <form onSubmit={async e=>{
   ...
   setVerificationError('');setVerificationStatus('');
   if(verificationFile.size>32768){setVerificationError('Choose a file no larger than 32 KB.');return;}
   setBusy(true);
   try{
   ...
   <input className="mt-4 block w-full text-sm" type="file" accept=".html,.xml" disabled={busy} aria-label="Choose webmaster verification file" onChange={e=>{setVerificationFile(e.currentTarget.files?.[0]??null);setVerificationStatus('');setVerificationError('');}} />
   <button type="submit" className={`${button} mt-4`} disabled={busy||!verificationFile}>{busy?'Please wait...':'Upload verification file'}</button>
  </form>
  {verificationError&&<p role="alert" className="mt-3 text-sm text-primary">{verificationError}</p>}
  {verificationStatus&&<p role="status" className="mt-3 text-sm text-accent">{verificationStatus}</p>}
  <h4 className="mt-4 text-sm font-semibold">Saved verification files</h4>
  {data.files.length===0&&<p className="mt-2 text-sm text-muted-foreground">No verification files have been saved yet. Choose a file and click Upload verification file.</p>}
  <ul className="mt-4 space-y-3">{data.files.map(f=><li key={f.filename} className="flex flex-wrap items-center justify-between gap-3"><a className="break-all text-sm text-accent" href={'/'+f.filename} target="_blank" rel="noreferrer">/{f.filename}</a><button type="button" disabled={busy} className={button} onClick={async()=>{setVerificationStatus('');setVerificationError('');setBusy(true);try{await viewerApi('registration/admin/seo/verification/delete',{filename:f.filename});await client.invalidateQueries({queryKey:['admin-seo']});setVerificationStatus(`${f.filename} removed.`);}catch(error){setVerificationError(error instanceof Error?error.message:'Could not remove the file.');}finally{setBusy(false);}}}>Remove</button></li>)}</ul>
 </div>
